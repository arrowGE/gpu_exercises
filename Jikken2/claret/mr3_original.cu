...
extern "C" __global__ 
void nacl_kernel_gpu(VG_XVEC *x, int n, int nat, VG_MATRIX *d_matrix, float xmax, float *fvec)
{
  int tid = threadIdx.x;
  ...
  for (j = 0; j < n; j++){
    inter(x[j].r, xi, fi, d_matrix, atypei + x[j].atype, xmax, xmax1);
  }
		 ...
		 double force[])
{
  ...
  nacl_kernel_gpu<<< grid, threads >>>(d_x,n,nat,d_matrix,xmaxf,d_force);
